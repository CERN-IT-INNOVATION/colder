...
import colder.core.quantum.sparse as cqs
...
class interface:
    ...
    def __compute_hamiltonian(self, hh : hamiltonian) -> scipy.sparse.spmatrix:
        """
    ...
    def compute_timeindependent_hamiltonians(self) -> List[scipy.sparse.spmatrix]:
        return [ self.__compute_hamiltonian(hh) for hh in self.hobjs ]
    
    def compute_timeindependent_hamiltonians_with_coeff(self) -> List[ Tuple[str,scipy.sparse.spmatrix] ]:
        return [ (hh.coeff, self.__compute_hamiltonian(hh)) for hh in self.hobjs ]
    
    def finalize(self, matrix : scipy.sparse.spmatrix) -> scipy.sparse.spmatrix:
        return matrix
