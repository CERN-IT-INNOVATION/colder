...
class interface:
    ...
    def __compute_hamiltonian(self, hh : hamiltonian) -> cupy.ndarray:
        """
    ...
    def compute_timeindependent_hamiltonians(self) -> List[cupy.ndarray]:
        return [ self.__compute_hamiltonian(hh) for hh in self.hobjs ]
    
    def compute_timeindependent_hamiltonians_with_coeff(self) -> List[ Tuple[str,cupy.ndarray] ]:
        return [ (hh.coeff, self.__compute_hamiltonian(hh)) for hh in self.hobjs ]
    
    def finalize(self, matrix : cupy.ndarray) -> cupy.ndarray:
        return matrix
